...
const Requests = () => {
  ...
  useEffect(() => {
    const fetchProfileData = async () => {
      try {
        ...
        const response = await fetch(`http://localhost:5000/api/reqevent/${recieverId}`, {
          headers: {
  ...
  useEffect(() => {
    const fetchDetails = async () => {
      try {
        ...
        const recieverId = GetUserInfoFromToken();
        const responseAnsvers = await fetch(`http://localhost:5000/api/ansverreq/reciever/${recieverId}`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
  ...
  const AnsverOnReq = async (ansver, reqId) => {
    try {
      const response = await fetch(`http://localhost:5000/api/reqevent/${reqId}`, {
        method: 'PATCH',
        headers: {
